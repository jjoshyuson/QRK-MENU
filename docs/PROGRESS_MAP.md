...
The approved Payment First pilot flow is implemented locally and enabled for Kusina Manila while it remains QRK Quick: customers must choose a payment method, cashless remains visibly disabled, pay-at-counter is usable, and submitted orders remain live until staff use the existing mode-aware completion action. This is a same-browser simulation only; provider selection, charging, reconciliation, refunds, and hosted payment metadata remain unresolved production work.
