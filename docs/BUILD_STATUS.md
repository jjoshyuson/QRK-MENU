...
- Reworked client creation after review into one compact ordered-gate builder. A client can enable Quick, Table or both; presets populate only the relevant journey, custom can start empty, and gates can be reordered by drag or the accessible move control, added, removed and configured inline without exposing every setting at once.
...
- Validation: `npm run check`, `git diff --check`, live QR token issue/resolve, all seven public entry variants, and a customer request appearing in the matching Client Admin Tables queue passed locally. The revised builder was verified with simultaneous Quick + Table, preset application, move/remove/add controls, collapsed configuration and a 390×844 viewport without horizontal overflow. No hosted or production database was changed.
