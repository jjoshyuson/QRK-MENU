...
For Kusina's QRK Quick payment-first flow, `View order` opens the normal review and fulfillment fields, then `Pay order` advances to a focused payment dialog. Two large cards make the next choice explicit: `Pay at the counter` is enabled, while `Cashless` remains disabled and labeled `Coming after the pilot`. Counter selection is retained on the device-local order and advances to the existing `Waiting for staff` confirmation. This simulates workflow only and does not execute or authorize a payment.
