...
function setupAccountMenu(profile,admin){
 profile.classList.add('account-trigger');profile.tabIndex=0;profile.setAttribute('role','button');profile.setAttribute('aria-haspopup','menu');profile.setAttribute('aria-expanded','false');profile.setAttribute('aria-label',`Open account menu for ${accessContext.displayName}`);
...
function renderOrders(){
 ...
 $('#active-order-count').textContent=received.length+preparing.length;$('#history-order-count').textContent=history.length;$('#received-order-count').textContent=received.length;$('#preparing-order-count').textContent=preparing.length;
...
if(dataService.mode==='supabase'){
 ...
 $('#orders-data-summary').textContent='Move new orders into preparation, then mark them served.';
}
