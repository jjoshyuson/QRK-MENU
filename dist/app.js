...
document.querySelectorAll('[data-settings-dialog]').forEach(button=>button.addEventListener('click',()=>{const dialog=$(`#${button.dataset.settingsDialog}`);dialog.showModal();requestAnimationFrame(()=>dialog.querySelector('input,select,button')?.focus())}));
...
function renderOrders(){
 const received=orders.filter(order=>order.status==='received').sort((a,b)=>new Date(a.createdAt)-new Date(b.createdAt)),preparing=orders.filter(order=>order.status==='preparing'||order.status==='ready').sort((a,b)=>new Date(a.createdAt)-new Date(b.createdAt)),history=orders.filter(order=>HISTORY_ORDER_STATES.includes(order.status)).sort((a,b)=>new Date(b.updatedAt||b.createdAt)-new Date(a.updatedAt||a.createdAt));
 $('#active-order-count').textContent=received.length+preparing.length;$('#history-order-count').textContent=history.length;$('#show-order-history').setAttribute('aria-label',`Open order history, ${history.length} item${history.length===1?'':'s'}`);$('#received-order-count').textContent=received.length;$('#preparing-order-count').textContent=preparing.length;
 $('#received-order-list').innerHTML=received.map(order=>orderCard(order,'Start preparing')).join('');$('#preparing-order-list').innerHTML=preparing.map(order=>orderCard(order,'Mark served')).join('');$('#history-order-list').innerHTML=history.map(historyCard).join('');
 $('#received-order-empty').hidden=received.length>0;$('#preparing-order-empty').hidden=preparing.length>0;$('#history-order-empty').hidden=history.length>0;
...
function showOrderStage(stage,{focus=false}={}){for(const name of ['received','preparing','requests'])$(`#${name==='requests'?'table-requests':name+'-orders'}-panel`).hidden=name!==stage;document.querySelectorAll('[data-order-stage]').forEach(button=>{const selected=button.dataset.orderStage===stage;button.classList.toggle('active',selected);button.setAttribute('aria-selected',String(selected));button.tabIndex=selected?0:-1});if(focus)requestAnimationFrame(()=>document.querySelector(`[data-order-stage="${stage}"]`)?.focus())}
function openOrderDetail(id){
 ...
 $('#cancel-order').hidden=final||(!admin&&!permission.cancelOrders);advance.hidden=final||!canAdvance;advance.disabled=false;advance.textContent=order.status==='received'?'Start preparing':'Mark served';
...
$('#advance-order').onclick=()=>{const order=orders.find(item=>String(item.id)===selectedOrderId);if(!order)return;if(order.status==='received')setOrderStatus(order,'preparing','Started preparing');else markOrderServed(order)};
