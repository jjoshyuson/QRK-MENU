...
async function tableApi(req,res,pathname){
  const parts=pathname.split('/').filter(Boolean),slug=decodeURIComponent(parts[2]||''),action=String(parts[3]||'').replace(/\/$/,'').toLowerCase(),sessions=tableSessions.get(slug)||[];
  ...
  if(action==='accept'){session.status='active';session.updatedAt=now;session.events.push({type:'session_accepted',at:now});sendJson(res,200,session);return}
