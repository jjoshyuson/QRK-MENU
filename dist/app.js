...
brandingForm.querySelector('.field-row').insertAdjacentHTML('afterend','<section class="table-setup-field"><div><strong>Dining tables</strong><p>Sets the numbered choices customers see when they select table service.</p></div><label for="business-table-count">Number of tables<input id="business-table-count" name="tableCount" type="number" min="1" max="200" inputmode="numeric" required></label></section>');
const tableCountInput=$('#business-table-count');
...
brandingForm.elements.businessName.value=currentBrand.businessName;tableCountInput.value=currentBrand.tableCount;renderBusinessBrand();
...
brandingForm.onsubmit=e=>{e.preventDefault();const tableCount=Math.max(1,Math.min(200,Number.parseInt(tableCountInput.value,10)||6));tableCountInput.value=tableCount;currentBrand=saveBusinessBrand(businessContext,{...currentBrand,businessName:brandingForm.elements.businessName.value.trim()||businessContext.businessName,tableCount,primary:brandingPrimary.value,nav:brandingNav.value});renderBusinessBrand();renderTableQrPreview();announce('Business profile saved for this client on this browser')};
...
const profileQrCard=$('.qr-card');profileQrCard.querySelector('.sample-link').insertAdjacentHTML('afterend','<section class="table-qr-preview" id="table-qr-preview"><div class="table-qr-heading"><div><strong>Table QR placeholder</strong><small>Choose a table to preview its built-in QR.</small></div><label for="table-qr-number">Table<select id="table-qr-number"></select></label></div><img id="table-qr-image" width="180" height="180" alt=""><code id="table-qr-link"></code></section>');
const tableQrSelect=$('#table-qr-number');$('#table-qr-preview').hidden=businessExperience.serviceMode!=='table';
function renderTableQrPreview(){const count=currentBrand.tableCount||6,selected=Math.min(Number(tableQrSelect.value)||1,count);tableQrSelect.innerHTML=Array.from({length:count},(_,index)=>`<option value="${index+1}"${index+1===selected?' selected':''}>${index+1}</option>`).join('');const url=new URL(buildCustomerMenuUrl(location.href,currentBrand.businessSlug));url.searchParams.set('table',selected);$('#table-qr-image').src=testQrImageUrl(url.href);$('#table-qr-image').alt=`Placeholder QR code for table ${selected}`;$('#table-qr-link').textContent=`Table ${selected} · ${url.pathname}${url.search}`}
tableQrSelect.onchange=renderTableQrPreview;renderTableQrPreview();
