...
The disposable hosted project `qrk-menu-development` now contains all six migrations, ten development identities, and published catalogs for all five businesses. GitHub Pages staging variables are configured. Hosted API evidence passes public menus, registered-device order creation, correct admin tenant reads for every business, and a customer Table request → staff acceptance → customer reconciliation → cleanup cycle. Deployment and physical two-device browser verification remain before calling staging operational.
...
| 1.1 Hosted development verification | In progress; API verified, deployment pending | Six migrations, five catalogs, ten Auth identities and safe Pages variables are installed in disposable hosted development | GitHub Pages deployment; physical devices | Business A denied Business B; customer orders and Table changes reconcile across two physical devices; clear/restore and photo rules pass |
...
6. Deploy the configured staging build to GitHub Pages and verify all five customer routes load their hosted menus.
7. Sign in on a separate staff device and prove live customer order plus Table request/acceptance reconciliation.
