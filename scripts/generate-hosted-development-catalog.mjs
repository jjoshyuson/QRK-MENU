...
const clients = DEVELOPMENT_CLIENTS.filter(client => !['kusina-manila', 'salamat'].includes(client.slug));
const prefixes = { 'salo-table': '31', 'tambay-tab': '41', 'ihaw-buffet': '51' };
const orderPrefixes = { 'salo-table': 'ST', 'tambay-tab': 'TC', 'ihaw-buffet': 'IB' };
...
for (const client of clients) {
  ...
  const businessId = uuid(prefix, 0, 1), adminId = uuid(prefix, 0, 101), menuId = uuid(prefix, 0, 201), revisionId = uuid(prefix, 0, 301), destinationId = uuid(prefix, 0, 801);
  const settings = JSON.stringify({ ...client.serviceProfile.settings, serviceMode: client.serviceProfile.settings.serviceMode });
  ...
  statements.push(`insert into public.menu_revisions (id,business_id,menu_id,version,state,published_at,created_by) values (${quote(revisionId)},${quote(businessId)},${quote(menuId)},1,'published',now(),${quote(adminId)}) on conflict (id) do nothing;`);
  const categories = [...new Set(client.menu.map(([category]) => category))];
  categories.forEach((name, index) => statements.push(`insert into public.categories (id,business_id,menu_id,revision_id,name,public_slug,sort_order) values (${quote(uuid(prefix, 0, 401 + index))},${quote(businessId)},${quote(menuId)},${quote(revisionId)},${quote(name)},${quote(slug(name))},${index}) on conflict (id) do nothing;`));
  client.menu.forEach(([category, name, price], index) => statements.push(`insert into public.menu_items (id,business_id,revision_id,category_id,stable_key,name,description,price_minor,available,sort_order) values (${quote(uuid(prefix, 0, 501 + index))},${quote(businessId)},${quote(revisionId)},${quote(uuid(prefix, 0, 401 + categories.indexOf(category)))},${quote(uuid(prefix, 0, 901 + index))},${quote(name)},'',${Number(price)},true,${index}) on conflict (id) do nothing;`));
