...
- Connected Review Order to the viewport bottom and redesigned its hierarchy: image-backed order rows use clean divider-only geometry, available dishes not already ordered appear in a compact “Popular with your order” rail, notes follow recommendations, and the larger primary checkout action anchors the sheet footer. Order thumbnails now open editing, compact steppers preserve row space, and a horizontal swipe reveals the destructive remove action without keeping Edit/Remove labels in every row.
