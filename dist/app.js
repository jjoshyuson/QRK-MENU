...
let categories=['Mains','Sides','Drinks'],active='All items',phoneActive='All',menuName='Main menu',editingId=null,nameMode='category',previewDevice='mobile',mobileCategory='All',mobileCustomer=false,toastTimer;
...
function renderMobileMenu(){
 ...
 $('#mobile-menu-sections').innerHTML=visible.map(c=>{
  ...
  return `<section class="mobile-menu-section"><div class="mobile-section-heading"><h3>${esc(c)}</h3><span>${dishes.length} item${dishes.length===1?'':'s'}</span></div><div class="mobile-dishes">${dishes.map(i=>`<article class="mobile-dish ${i.available?'':'is-sold-out'}"><div class="mobile-dish-content"><div><h4>${esc(i.name)}</h4><p>${esc(i.description)}</p></div>${mobileCustomer?'':`<button class="mobile-icon-control" data-edit="${i.id}" aria-label="Edit ${esc(i.name)}"><span data-icon="edit"></span></button>`}</div><div class="mobile-dish-footer"><strong>${price(i.price)}</strong>${mobileCustomer?(i.available?'':'<span class="mobile-sold-label">Sold out</span>'):`<button class="mobile-stock-button" type="button" role="switch" aria-checked="${i.available}" data-mobile-stock="${i.id}" aria-label="${esc(i.name)} available"><span class="mobile-stock-dot"></span>${i.available?'Available':'Sold out'}</button>`}</div></article>`).join('')}${mobileCustomer?'':`<button class="mobile-add-in-category" data-mobile-add="${esc(c)}"><span data-icon="plus"></span>${dishes.length?'Add to '+esc(c):'Add your first item'}</button>`}</div></section>`;
