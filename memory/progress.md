...
- The disposable hosted Supabase project `qrk-menu-development` has all six repository migrations, ten forced-password-change identities, and published catalogs for all five development businesses. GitHub Pages holds only the safe hosted URL and publishable key. API checks pass menu reads, device registration, order creation and tenant-authenticated admin visibility for every business; Salo also passes customer Table request, staff acceptance, customer reconciliation and cleanup. Deployment plus physical two-device checks are next.
