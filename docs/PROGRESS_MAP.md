...
Client workspaces now remember the last permitted section per business/user identity in the same browser. Unseen Client Admins still open Dashboard, valid deep links take initial precedence, and inaccessible or stale values return through the existing role-aware fallback without crossing tenant or account boundaries. This candidate remains pending QA, local user review, and explicit deployment approval.
...
The disposable hosted project `qrk-menu-development` contains all ten migrations and exactly ten canonical development businesses, each with an ordered service-gate configuration and a 50-item published catalog. Ten development identities remain attached to the original five login-enabled businesses; the other five are catalog/service-profile fixtures. GitHub Pages staging deployment `34911305544` succeeded, and the live Kusina route rendered its hosted database catalog with no privileged key in the public configuration. Prior hosted API evidence passes public menus, registered-device order creation, correct admin tenant reads for the five login-enabled businesses, and a customer Table request → staff acceptance → customer reconciliation → cleanup cycle. Physical two-device browser verification and runtime smoke checks for the five new catalog-only routes remain before calling staging fully proven.
...
| 0.30 QRK brand and UI/UX review | Current; approved local work | Clean three-part logo set, global light/dark presets, business-customizable public-menu backgrounds, a documented compact visual system, refined public dish cards, heading-free Settings actions, and single-action staff order completion | Integrated demo; supplied logo, Settings, Menu, and Orders directions | `AGENT/STYLE.md` guides future UI work; Business Profile controls a scoped menu wallpaper and readability veil; public browsing cards prioritize photo, name and price with a compact plus cue overlapping the outer top-right corner, while descriptions stay in the item sheet; Settings preserves every control through focused editors; Quick orders move from Received to Paid, Table orders move from Received to Served, and Table session work remains under Tables; representative layouts have no horizontal overflow; user accepts the direction |
...
| 0.33 Shared component source | Implemented locally; visual regression review pending | One component variable/visual layer drives owner, admin, staff, customer and catalog UI; stable Inspect Element labels expose names and variants | Existing production UI and design tokens | All operational routes load the shared layer and registry; fifteen families render at phone, tablet and desktop widths; representative flows retain their accepted appearance and behavior |
...
| 1.09 Table presets and session-entry contract | Current; LAN and hosted-preview parity proven | Five development clients compose eight service layers; Table count is configurable per business, with roomy selection through 10 and a compact number grid above 10; QR links can lock a specific multi-digit table; occupied-table joining, first-guest approval, cancellable/time-limited waits and staff-controlled reopening remain intact | Quick/Table demo tenants; LAN server or same-browser hosted preview | Salamat demonstrates 20 selectable tables and a placeholder table-specific QR route; LAN browser sessions share table occupancy and join approval; hosted customer/staff tabs reproduce preset rules on one browser origin; durable cross-device hosting and trusted production QR identity remain required |
...
| 1.1 Hosted development verification | In progress; deployed and API verified | Six migrations, five catalogs, ten Auth identities and safe Pages configuration are live | Physical devices | Business A denied Business B; customer orders and Table changes reconcile across two physical devices; clear/restore and photo rules pass |
| 1.11 Order source of truth and recovery | Hosted schema deployed; runtime verification pending | PostgreSQL-backed device identity, submitted orders, operational archive batches, and restoration of the newest two clears | Hosted Auth/runtime config; physical devices | Hosted two-device evidence passes; restaurant/menu/product data remains unchanged through clear/restore |
...
6. Open the deployed customer QR on a physical phone and sign in on a separate staff device.
7. Prove live customer order plus Table request/acceptance reconciliation across those devices and networks.
