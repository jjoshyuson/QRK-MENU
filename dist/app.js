...
function itemThumbnail(i){return i.photo?`<img class="item-stamp" src="${esc(i.photo)}" alt="" loading="lazy" decoding="async" width="47" height="51">`:stamp(i)}
...
const tenantAppearance=$('.appearance-card');tenantAppearance.remove();const profilePage=$('[data-dashboard-page="profile"]');profilePage.querySelector('.page-heading .subtitle').textContent='Update this client’s business details and logo.';profilePage.querySelector('.logo-field p').textContent='PNG, JPG, or WebP, up to 2 MB.';
...
function renderBusinessBrand(){applyBusinessBrand(currentBrand);const name=currentBrand.businessName||businessContext.businessName||'Business';document.querySelector('.business strong').textContent=name;paintBusinessLogo(document.querySelector('.business-icon'),true);paintBusinessLogo($('#business-logo-preview'));paintBusinessLogo($('#brand-preview-mark'),true);if($('#brand-preview-name'))$('#brand-preview-name').textContent=name;$('#qr-business-name').textContent=`${name} menu`;const cover=$('.restaurant-cover');if(cover){cover.querySelector('h2').textContent=name;cover.querySelector('p').textContent=businessExperience.description;cover.querySelector('.restaurant-monogram').textContent=`${name.trim().charAt(0).toUpperCase()}.`;cover.querySelector('.restaurant-detail').innerHTML=`₱ PHP <i>•</i> ${businessExperience.serviceMode==='table'?'Table service':'Quick service'}`};brandingPrimary.value=currentBrand.primary;brandingNav.value=currentBrand.nav;$('#remove-business-logo').hidden=!currentBrand.logoDataUrl;document.querySelectorAll('.color-swatch').forEach(swatch=>{const selected=swatch.dataset.themeColor.toLowerCase()===currentBrand.primary.toLowerCase();swatch.classList.toggle('selected',selected);swatch.setAttribute('aria-pressed',String(selected))})}
