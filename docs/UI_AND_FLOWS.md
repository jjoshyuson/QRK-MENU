...
The shared Quick/Table staff surface is a service-first workspace with one uninterrupted three-column stage bar: Received, Preparing, and Tables. Only the selected stage fills the operational area, keeping staff focused on one service state at a time. Every order card carries its immediate action: Start preparing acknowledges a received order, while Mark served completes the internal ready/completed transition and moves the order out of the live workspace. History is a separate top-right view containing served and cancelled orders; on responsive layouts its control sits directly beside the hamburger inside the shared top-bar action group. The responsive detail panel keeps fulfillment, items, selected options, notes, totals and cancellation without exposing development verification steps in the primary workflow.
