...
(() => {
  ...
  const open = (input, trigger) => {
    ...
    wheel.addEventListener('scroll', () => { if (!active || active.layer !== layer) return; clearTimeout(active.scrollTimer); active.scrollTimer = setTimeout(() => { if (layer.isConnected) nearest(); }, 80); }, {passive:true});
    wheel.querySelectorAll('.qrk-number-option').forEach(option => option.onclick = () => { selectValue(input, wheel, Number(option.dataset.value)); option.scrollIntoView({block:'center'}); close(); });
    layer.querySelector('.qrk-number-backdrop').onclick = () => close();
    layer.addEventListener('keydown', event => {
      const value = Number(input.value), next = event.key === 'ArrowDown' ? Math.min(max, value + 1) : event.key === 'ArrowUp' ? Math.max(min, value - 1) : null;
      if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); close(); }
      else if (next !== null) { event.preventDefault(); const option = wheel.querySelector(`[data-value="${next}"]`); selectValue(input, wheel, next); option?.scrollIntoView({block:'center'}); option?.focus(); }
    });
    requestAnimationFrame(() => { const selected = wheel.querySelector('[aria-selected="true"]'); selected?.scrollIntoView({block:'center'}); selected?.focus({preventScroll:true}); });
  };
