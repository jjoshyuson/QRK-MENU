...
- Tightened Business Details to equal-width, left-aligned label/value columns, including compact address and description rows, and raised the dining-table guidance contrast. Table count now uses a shared iOS-style scroll-snap number wheel backed by the original bounded numeric field, eliminating desktop spinner arrows without changing validation or saved data.
