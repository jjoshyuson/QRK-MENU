...
- The item confirmation check is optically centered, item and public-menu utility sheets now mirror their short slide entrance on dismissal, and Review Order plus Order History use the same vertical entrance. The fulfillment chooser uses one stable top-left back control, preventing its title column from collapsing on narrow screens.
- Final item-sheet direction keeps the established option cards and typography while adopting only three photo-led details from the reference: a large hero image, an always-visible Special request field, and a bottom Add to order or Update item pill with the live calculated price. The intermediate Customize item title and check-confirm control are removed.
...
## September 15, 2026 — Configurable business service gates

- Added one normalized business service profile built on the Quick/Table foundation. Quick now stores consumption, fulfillment, payment and conditional locator gates; Table stores entry, party, bundle, payment, entitlement/refill, time/quantity, joining/host, bill/closure and two-hour inactivity-warning rules.
- Reworked new and existing client settings after review into one compact grouped list with animated slide-up editors. Quick and Table are independent checkboxes and may both be enabled; each enabled service gets its own preset and ordered-gate sheet, while each gate opens a focused sub-sheet instead of expanding another nested card. Presets populate only the relevant journey, custom can start empty, and gates can be reordered by drag or the accessible move control, added and removed.
- Replaced the reviewed sheet's opaque gray treatment and browser-native dropdown menus with a charcoal glass surface and custom choice popovers. The popovers retain draft selection in the existing workflow, show the selected option with an Apple-blue check, dismiss outside or with Escape, stay within `48vh`, center on narrow screens, and use a short spring reveal with a reduced-motion fallback.
- Promoted the approved grouped-settings and material-sheet shell into the shared component layer. `QrkSheet` now owns responsive floating/bottom geometry, backdrop and Escape dismissal, focus containment/restoration, and motion; DA client configuration owns its data snapshot so Done commits while Close, backdrop, or Escape restores the prior draft. The component catalog includes grouped-list and sheet specimens for staged reuse by Client Operations.
- Refined the material hierarchy after visual review: choice triggers, popovers, grouped sections, gate lists and sheet action buttons are borderless at rest, while faint separators remain only between peer rows and keyboard focus retains a clear outline. The shared popover uses a lighter translucent material instead of an opaque near-black slab.
- Added the shared material field-list treatment used by Business details, Client Admin and gate configuration: labels sit left, editable values sit right, and fields remain transparent and borderless inside one dark tonal surface. Autofill, keyboard-focus and invalid-state rules prevent default white browser inputs from returning without hiding interaction state.
- Consolidated the development fixture inventory at exactly ten clients. `DEVELOPMENT_CLIENTS` now drives browser fallback plus generated hosted business records, profiles, 500 menu items and ordered `business_service_configs`, covering Quick, direct/traditional Table, prepaid and pay-later buffet, timed service, quantity allowance, minimum spend, deposit entry, host/guest approval and bill/closure behavior.
- Added working local examples for host-controlled Open Tab, prepaid buffet, pay-later buffet, timed buffet, limited-quantity package, minimum-spend table and deposit-required table while retaining the original Quick and Table clients.
- QRK Admin exposes the gates during client creation. Client workspace routing now honors local preview sessions, and public menus derive entry fields and session terms from capabilities instead of preset names.
- Added server-validated permanent table-entry tokens. The browser removes the token from the visible URL immediately after authorization; no service/API secret is sent to the customer. Active-session inactivity moves to `inactivity_warning` with grace elapsed as an attention flag and never releases or labels the table clean.
- Validation: `npm run check`, `git diff --check`, live QR token issue/resolve, all seven public entry variants, and a customer request appearing in the matching Client Admin Tables queue passed locally. The shared-sheet candidate was verified for draft discard/commit, Escape dismissal and focus return, nested gate navigation, custom popover selection, and overflow/geometry at 390×844, 768×1024, and 1280×800. No hosted or production database was changed.
- Hosted-development follow-up: the idempotent generated catalog was applied only to `qrk-menu-development`. A public-schema-only verification dump returned exactly 10 businesses, 10 `business_service_configs` rows and 500 items in published revisions; production was not connected or changed. The five newly added catalog/service fixtures do not yet have hosted Auth identities.
...
- Corrected the shared live-menu selector after the public categories moved inside the unified `.menu-tools` toolbar; owner and customer category controls now receive the same shared pill-radius token from the final component layer.
