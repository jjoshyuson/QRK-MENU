...
| 1.096 Customer device simulation lab | Complete on `codex/testing-simulations`; publish not requested | An unlinked test route runs ten stable customer phone identities with business presets, pasted URLs, configurable grid density and pagination | Customer menu route; scoped device storage; shared demo order/session contracts | Isolation contract passes; browser checks cover preset switching, paging, and representative phone/tablet/desktop widths |
