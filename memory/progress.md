...
- The disposable hosted Supabase project `qrk-menu-development` is linked and rebuilt from all five repository migrations plus the development seed. Hosted migration history matches Git and database lint passes. Ten hosted development identities now provide one Client Admin and one Client Staff account for each of the five preview businesses; all memberships are active and all accounts require a first-login password change. Temporary credentials are gitignored. The exact next milestone is safe public runtime configuration and login/tenant verification, followed by full hosted menus for the three added tenant shells, recovery and two-device exit evidence.
