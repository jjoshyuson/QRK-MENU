...
- Reduced public-menu backdrop blur from 12px to 6px and added a fast 90ms closing transition from blur to sharp, avoiding the previous abrupt background snap without delaying sheet dismissal.
- Set Review Order to a consistent 90dvh card with a flexible scrollable content region, keeping its header and bottom action in place and intentionally preserving blank breathing room when an order is short.
- Connected Review Order to the viewport bottom and redesigned its hierarchy: image-backed order rows use clean divider-only geometry, available dishes not already ordered appear in a compact “Popular with your order” rail, notes follow recommendations, and the larger primary checkout action anchors the sheet footer. Order thumbnails open editing, compact steppers preserve row space, and releasing a left swipe beyond 45% dismisses and removes the row in one gesture while shorter swipes snap back. Active drags lock their axis once so minor vertical drift cannot interrupt a horizontal return; reversing a left swipe can pull the row up to 24px past rest with resistance before a spring return. Snap-back and dismissal alone animate, and the delete surface stays vertically inset so row padding remains visible.
- Removed live backdrop filtering from every public-menu dish card while retaining its translucent fill, rim highlight, and lighter static shadow; replaced scroll-time section geometry scans with `IntersectionObserver`; and constrained the recommendation rail with mandatory snap stops and no inline overscroll bounce. Menu Studio now creates a separate 480px/72KB card image alongside the preserved 960px hero image, and customer grids, cart thumbnails, and recommendations prefer that card-sized source.
