...
| 0.30 QRK brand and UI/UX review | Current; approved local work | Clean three-part logo set, global light/dark presets, business-customizable public-menu backgrounds, a documented compact visual system, simplified public dish tiles, heading-free Settings actions, and single-action staff order completion | Integrated demo; supplied logo, Settings, Menu, and Orders directions | `AGENT/STYLE.md` guides future UI work; Business Profile controls a scoped menu wallpaper and readability veil; public browsing tiles show only photo, name, and price; Settings preserves every control through focused editors; Quick orders move from Received to Paid, Table orders move from Received to Served, and Table session work remains under Tables; representative layouts have no horizontal overflow; user accepts the direction |
