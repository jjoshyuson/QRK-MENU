...
- Editable sheet values live inside the same dark tonal grouped surface as their labels. Prefer compact label-left/value-right rows, with one subtle divider between peers. Inputs, textareas and selects must merge into the row: never use a white fill, default browser bevel, independent rim or nested field card inside a material sheet. Preserve a visible caret and selection, and show an outline only for keyboard focus or validation/error state.
