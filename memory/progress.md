...
- Quick Review Order uses immediate Pickup/Serve at table cards with no preselection or primary action until selection. Existing table selection remains, while active Table/Open Tab sessions derive fulfillment without asking again. Responsive browser evidence covers 390×844, 768×1024, and 1280×800; the user approved deployment after local review.
...
- The disposable hosted Supabase project `qrk-menu-development` has all six repository migrations, ten forced-password-change identities, and published catalogs for all five development businesses. GitHub Pages deployment `34911305544` succeeded with only the safe hosted URL and publishable key; a live-browser Kusina reload rendered the database catalog. API checks pass menu reads, device registration, order creation and tenant-authenticated admin visibility for every business; Salo also passes customer Table request, staff acceptance, customer reconciliation and cleanup. Physical two-device checks are next.
- The corrected order-sync migration also rebuilds locally, local lint passes, and all 40 pgTAP tests pass, including registered-device ordering and clear/restore coverage.
...
- QRK Admin now has functional local Overview, Clients and Client Admins views. It can create and pause/activate browser-local client portals, generate the initial Client Admin's one-time credentials, and route that account through the existing first-login password gate. QRK Admin has no Client Staff controls. The five fixed development tenants now have hosted admin/staff accounts, but dynamic trusted provisioning from QRK Admin remains incomplete.
