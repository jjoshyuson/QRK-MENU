...
- The disposable hosted Supabase project `qrk-menu-development` is linked and rebuilt from all five repository migrations plus the development seed. Hosted migration history matches Git and database lint passes. The exact next milestone is to configure hosted Auth and safe public runtime values, then complete tenant, recovery and two-device exit evidence before calling synchronization operational.
