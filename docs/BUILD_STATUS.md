...
- Validation: `npm run check`, `git diff --check`, live QR token issue/resolve, all seven public entry variants, and a customer request appearing in the matching Client Admin Tables queue passed locally. The revised settings flow was verified for new and existing clients, simultaneous Quick + Table, preset and ordered-gate sheets, focused gate configuration, custom popover selection/dismissal, and a full-width 390×844 dialog without horizontal overflow. No hosted or production database was changed.
