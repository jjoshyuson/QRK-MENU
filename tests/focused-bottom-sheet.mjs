...
assert.match(html, /ui-components\.css\?v=14[\s\S]*cart-dock\.css\?v=24/);
assert.match(customerCss, /dialog\.focused-sheet::backdrop,[\s\S]*dialog\.quick-service-dialog::backdrop,[\s\S]*dialog\.table-entry::backdrop\s*\{[^}]*background:\s*rgba\(248, 250, 252, \.16\)[^}]*backdrop-filter:\s*blur\(12px\) saturate\(\.85\)/s);
