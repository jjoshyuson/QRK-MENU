...
function defaultState(slug){
  ...
  const items=(client?.menu||[]).map(([category,name,price,description,photo],index)=>({id:index+1,category,name,description,price:price/100,photo,available:true,hidden:false,options:''}));
  return{menuName:'Main menu',categories:[...new Set(items.map(item=>item.category))],items,optionGroups:slug==='kusina-manila'?kusinaOptionGroups.map(group=>({...group,choices:group.choices.map(choice=>({...choice}))})):[]};
}
